defmodule RompCrmWeb.UserAuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, RompCrmWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "require_authenticated_user/2" do
    ...
    test "does not set login flash when redirecting unauthenticated root request", %{conn: conn} do
      conn =
        %{conn | path_info: [], query_string: ""}
        |> fetch_flash()
        |> UserAuth.require_authenticated_user([])
