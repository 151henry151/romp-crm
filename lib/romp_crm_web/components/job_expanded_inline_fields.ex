  ...
  def job_expanded_inline_fields(assigns) do
    ...
    <div class={@wrap}>
      <%= if @can_edit_jobs do %>
        ...
        >
          <:readonly>
            <span class="whitespace-pre-wrap break-words text-base-content">
        ...
        >
          <:readonly>
            <%= if @job.scheduled_on do %>
              {Date.to_iso8601(@job.scheduled_on)}
            <% else %>
              —
            <% end %>
          </:readonly>
      ...
      <% else %>
        ...
        <p>
          <span class="font-medium text-base-content/90">Status:</span> {status_label(@job.status)}
        </p>
        <p>
