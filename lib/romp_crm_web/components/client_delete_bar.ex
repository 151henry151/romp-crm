defmodule RompCrmWeb.ClientDeleteBar do
  @moduledoc false
  use Phoenix.Component

  attr :client, :any, required: true
  attr :delete_step, :integer, required: true
  attr :job_count, :integer, required: true
  attr :can_edit, :boolean, required: true

  def client_delete_bar(assigns) do
    ~H"""
    <%= if @can_edit do %>
      <div class="pt-3 mt-2 border-t border-base-300 space-y-2">
        <%= if @delete_step == 1 do %>
          <div class="rounded-lg border border-amber-400/60 bg-amber-500/10 px-3 py-2 text-xs text-amber-950 dark:text-amber-100">
            Step 1 of 3: This client has {@job_count} linked job(s). Deleting the client will permanently delete all linked jobs and their hours, materials, and photos.
          </div>
        <% end %>
        <%= if @delete_step == 2 do %>
          <div class="rounded-lg border border-red-500/50 bg-red-500/10 px-3 py-2 text-xs text-red-900 dark:text-red-100">
            ...
            <span class="font-semibold">{@client.client_name}</span>, all {@job_count} job(s), and everything on those jobs.
          </div>
        <% end %>
        <div class="flex flex-wrap items-center gap-2">
          <%= if @delete_step < 2 do %>
            <button
              type="button"
              phx-click="client_delete_advance"
          ...
          <% else %>
            ...
            >
              Delete client and all jobs
            </button>
          <% end %>
          <%= if @delete_step > 0 do %>
            <button
              type="button"
              phx-click="client_delete_cancel"
              phx-value-id={@client.id}
