defmodule JgsCrm.Businesses do
  @moduledoc """
  ...
  alias JgsCrm.Businesses.{Business, BusinessInvitation, BusinessMembership}
    ...
    Repo.transaction(fn ->
      with {:ok, business} <-
           ...
           {:ok, _mem} <-
             ...
             |> Repo.insert() do
        _ = JgsCrm.Accounts.maybe_set_default_sms_business(user, business.id)
        business
      else
  ...
  defp insert_invitation_and_mail(%Business{} = business, %User{} = inviter, email) do
    ...
    case %BusinessInvitation{}
      ...
      {:ok, invitation} ->
        ...
        _ = JgsCrm.Businesses.Notifier.deliver_invitation(invitation, business, url)
        {:ok, invitation}
